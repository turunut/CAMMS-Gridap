...
#reffeR = ReferenceFE(lagrangian,VectorValue{1,Float64},order-1)
...
# Definim l'integration mesh
Ωᵣ = Triangulation(model)
# Contruim el l'espai de mesura de Lebesgues de ordre "degree"
dΩᵣ = Measure(Ωᵣ,degree-1)
...
a((u,ω,θ),(v,w,t)) = ∫( ∂(v) ⊙ σₑ(CTf[1],∂(u)) + ∂(t) ⊙ σₑ(CTf[2],∂(θ)) )*dΩ + # Axial         + Axial/Bending
                     ...
                     ∫( γ(MT,∇(w),t) ⊙ σₑ(CTf[4],γ(MT,∇(ω),θ)) )*dΩᵣ # Shear
...
S((u,ω,θ),(v,w,t)) = ∫( γ(MT,∇(w),t) ⊙ σₑ(CTf[4],γ(MT,∇(ω),θ)) )*dΩᵣ
